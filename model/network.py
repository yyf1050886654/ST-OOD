...
torch.manual_seed(42)  # 设置随机种子，可以使用不同的整数值
...
class Bi3DOF(nn.Module):
    ...
    def encode(self, x, nsamples=1):
        # print("self.encoder.encode(x, nsamples):",self.encoder.encode(x, nsamples))
...
class GaussianEncoderBase(nn.Module):
    ...
    def reparameterize(self, mu, logvar, nsamples):
        batch_size, nz = mu.size()
        std = logvar.mul(0.5).exp()
        mu_expd = mu.unsqueeze(1).expand(batch_size, nsamples, nz)
        std_expd = std.unsqueeze(1).expand(batch_size, nsamples, nz)
        # eps = torch.zeros_like(std_expd).normal_()
        eps = torch.randn((batch_size, nsamples, nz), device=mu.device)
