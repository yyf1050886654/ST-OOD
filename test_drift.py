...
def load_model(test_config):
    ...
    # args.n_seqs = [6 for f in test_config["frames_per_clip"]]  # New: changed to list # Changed to test all sequences(aka window) in video(aka trace). Number of sequences(aka windows) is total frames-frame size(aka nd = 16)
    # args.n_seq = min([frame_len - args.nd for frame_len in test_config["frames_per_clip"]])
    args.n_seq = 41 #41
    args.n_seqs = [args.n_seq for f in test_config["frames_per_clip"]]
    args.number_windows = [22 for f in test_config["frames_per_clip"]]
    # Load  weights
    encoder = Encoder(args)
    decoder = Decoder(args)
    model = Bi3DOF(encoder, decoder, args).to(args.device)
    # print("test_config['model_file']: ", test_config["model_file"])
...
bi3dof_simple_test_in = {
    "model_file": lambda model_save_folder: "{}/bi3dof-simple-600epoch-6seq-seed{}.pt".format(model_save_folder, SEED),
    # "model/nuscenes-mini/bi3dof-simple-600epoch.pt",
    "network": "simple",
...
def getOutBi3DOF(type_of_OOD):
    ...
    bi3dof_simple_test_out = {
        "model_file": lambda model_save_folder: "{}/bi3dof-simple-600epoch-6seq-seed{}.pt".format(model_save_folder,
                                                                                                  SEED),
        # "model/nuscenes-mini/bi3dof-simple-600epoch.pt",
        "network": "simple",
...
def run(type_of_OOD):
    ...
    for idx, bi3dof_simple in enumerate(
            [bi3dof_simple_test_in, getOutBi3DOF(type_of_OOD)]):  # i.e. for traces in [iD traces, OOD traces]
    ...
    try:
        ...
        pass
    if args.nd == 16:  # orig default value
        ...
        print(f'Clip len ablation: cl={args.nd}')
        np.save(f'./npz_saved/drift_in_NTU_cl_{args.nd}.npy', scores_of_only_in_points)
        np.save(f'./npz_saved/drift_out_NTU_cl_{args.nd}.npy', scores_of_only_out_points)
